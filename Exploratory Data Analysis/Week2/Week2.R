...
q<-transform(q,Month=factor(Month))

q$Month <- factor(q$Month, labels = c("Mayo","Junio","Julio",
                                      "Agosto","Septiembre")

xyplot(Ozone ~ Wind | Month, data = q, layout = c(5, 1),grid=T)

#The type = argument can be used to enhance the figure with data-responsive elements. 
#On the left, we specify the default value type = "p", which requests only points, 
#and on the right, we request a simple linear regression with type = c("p", "r").

xyplot(Ozone ~ Wind | Month, data = q, 
       layout = c(5, 1),
       grid=T,
       type = c("p", "r"))
